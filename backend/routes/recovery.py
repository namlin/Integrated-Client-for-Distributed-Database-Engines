...
def run_recovery(tid: str, payload: RecoveryRequest):
    try:
        txn = transaction_manager.get_transaction(tid)
        if not txn:
            raise HTTPException(status_code=404, detail=f'Transaction {tid} not found')

        # Try to get the live adapter for the transaction's engine
        adapter = None
        engine_id = txn.get('engine_id')
        if engine_id:
            adapter = db_manager.get_adapter(engine_id)
